...
# Alembic Claude Code Workflow

Use this skill when the user explicitly asks Claude Code to work with Alembic setup, status, diagnostics, bootstrap/rescan workflows, local knowledge, Guard checks, or Alembic itself.
...
If status or diagnostics says the project root is unresolved or points inside the Claude Code plugin cache, pass the current workspace directory as the `projectRoot` argument on subsequent Alembic tool calls. `projectRoot` must be an absolute path; without it, Alembic project workflows cannot run.

If the workspace is not initialized and the user wants Alembic knowledge for this project, call `alembic_init`. The default profile is Ghost mode, so Alembic data is stored in the external workspace data root and Claude Code does not write IDE configuration into the project.
...
Use `alembic_bootstrap` for default Claude Code host-agent cold start and `alembic_rescan` for host-agent refresh. Claude Code reads the Mission Briefing, analyzes the project, submits knowledge, and completes dimensions; this path does not require an Alembic AI Provider.
...
Use `alembic_project_skill` for Project Skill source storage, refresh, receipt, export, and Claude Code runtime visibility. The old Claude Code-facing `alembic_skill` alias has been removed from the Plugin tool surface.

- `list` shows built-in skills, dataRoot source skills, Plugin delivery receipts, Claude Code runtime exports, and the effective winner.
...
Set `authorizeProjectSkillExport: true` only when the user has approved making that Project Skill visible in this project. A blocked response with `authorizationStatus: "pending"` means the receipt is valid but Claude Code has not been authorized to write `.agents/skills`.
...
Default Claude Code mode is agent tier. It may search knowledge, prime tasks, run Guard, use host-agent bootstrap/rescan, and submit candidates. Explicit daemon jobs may require Alembic resident-service configuration, but the Claude Code plugin does not configure third-party AI providers or store API keys.
